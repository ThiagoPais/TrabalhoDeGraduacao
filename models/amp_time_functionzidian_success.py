...
def amp_time (file_path,visualize):
    ...
    def check_and_fill_samples(y, target_samples=125, total_target_samples=1500):
        ...
        if total_samples < total_target_samples:
            # caculate missing samples
            missing_samples = total_target_samples - total_samples
            last_valid_samples = y[-target_samples:]  # get the last valid sample
            # fullfill the missing samples
    ...
    with open(file_path, 'r') as csvfile:
        ...
        for k in range(total_frames):
            ...
            for j in range(N10s * k, N10s * (k + 1), NS):  # One 10 seconds Frame
                ...
                for i in range(NS):
                    ...
                    # #cvv.append([float(item) for item in y[0 + j][1:-1].split(',')])  # No.1 Sensor
                    # cvv.append([float(item) for item in y[2 + j][1:-1].split(',')])  # No.3 Sensor
                    # #cvv.append([float(item) for item in y[3 + j][1:-1].split(',')])  # No.4 Sensor
                    # #cvv.append([float(item) for item in y[4 + j][1:-1].split(',')])  # No.5 Sensor
                ...
                #######   remove pilot   #############
                idxRm = [26, 53, 80, 107, 134, 161, 188, 215, 242, 269, 296, 323, 350, 377, 404, 431, 458, 485, 512,
                         539]
        ...
        time_axis = np.linspace(0, total_frames * 10, total_amp.shape[1])
        ...
        for k, dir_name in enumerate(directions):
            ...
            if visualize:
                ...
                time_axis = np.linspace(0, 10, len( avg_amp_for_direction))  # each direction for 10s
                plt.plot(time_axis,  avg_amp_for_direction)

                plt.tick_params(axis='both', labelsize=28)
                plt.ylabel('|CSI|', fontsize=28)
                plt.xlabel('Time (s)', fontsize=28)
                plt.title(f"Amplitude over Time for All Directions", fontsize=28)
                plt.show()
                plt.close()
...
#     file_in = 'D:/KIT/毕业设计/Boyang-Master-thesis/data/0929itiv 127/insert/i' + file_in
#
#
